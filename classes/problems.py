...
class RationalExponentPractice(ProblemBase):
    def __init__(self, num_quest: int, nrange: tuple[int, int], erange: tuple[int, int], *types: str, var: tuple[str, ...] = tuple()):
        ...
        uppercase parameters are drawn from erange, x and y are variables):
         ...
         - simp_multdivdist: simp_multdiv but raised to a fractional exponent; denominators have a 50% chance of being 1
         ...
         - simp_allin: \frac{(ax^(A/B)y^{C/D})^{E/F}\sqrt[G]{x^H y^I}}{(\sqrt[J]{x^K y^L})^{M/N}};
    ...
    def get_problem(self) -> list[Math]:
        ...
        match prob_type:
            ...
            case 'simp_allin':
                x, y = sample(self.var, 2)
                # \frac{(ax^(A/B)y^{C/D})^{E/F}\sqrt[G]{x^H y^I}}{(\sqrt[J]{x^K y^L})^{M/N}}
                a = randint(self.nrange[0], self.nrange[1])
                while a == 0:
                    a = randint(self.nrange[0], self.nrange[1])
                num_left = rf"\left( {Term(f'{x}^{{{generate_exponent().dumps()}}}{y}^{{{generate_exponent().dumps()}}}', a, 1).dumps()} \right)^{{{generate_exponent().dumps()}}}"
